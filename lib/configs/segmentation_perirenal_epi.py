...
class SegmentationPerirenal(TaskConfig):
    def __init__(self):
        super().__init__()
    ...
    def init(self, name: str, model_dir: str, conf: Dict[str, str], planner: Any, **kwargs):
        ...
        # Others
        self.epistemic_enabled = strtobool(conf.get("epistemic_enabled", "true")) # Epistemic enabled
        self.epistemic_samples = int(conf.get("epistemic_samples", "2")) # MC 2 samples
        logger.info(f"EPISTEMIC Enabled: {self.epistemic_enabled}; Samples: {self.epistemic_samples}")

        self.tta_enabled = strtobool(conf.get("tta_enabled", "false")) # TTA disabled
        self.tta_samples = int(conf.get("tta_samples", "5"))
        logger.info(f"TTA Enabled: {self.tta_enabled}; Samples: {self.tta_samples}")

    def infer(self) -> Union[InferTask, Dict[str, InferTask]]:
        task: InferTask = lib.infers.SegmentationFat(
            path=self.path,
    ...
    def trainer(self) -> Optional[TrainTask]:
        ...
        task: TrainTask = lib.trainers.SegmentationFat(
            model_dir=output_dir,
            network=self.network,
            description="Train Fat Segmentation Model",
            load_path=load_path,
    ...
    def scoring_method(self) -> Union[None, ScoringMethod, Dict[str, ScoringMethod]]:
        ...
        if self.epistemic_enabled:
            methods[f"{self.name}_epistemic"] = EpistemicScoring(
                ...
                transforms=lib.infers.SegmentationFat(None).pre_transforms(),
